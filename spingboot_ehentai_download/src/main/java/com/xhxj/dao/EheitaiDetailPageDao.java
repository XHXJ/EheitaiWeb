...
public interface EheitaiDetailPageDao extends JpaRepository<EheitaiDetailPage, Integer>, JpaSpecificationExecutor<EheitaiDetailPage> {
    ...
    //根据传入的gid去查询这个作品实际的下载数量
    @Query(value = "SELECT COUNT(page)  FROM eheitai_detail_page WHERE eheitai_id = ? ", nativeQuery = true)
    Integer findByGidWherePage(Integer gid);

    //根据gid去查询作品的图片链接
    //SELECT img_url FROM eheitai_detail_page WHERE eheitai_id = '452182'
    @Query(value = "SELECT url FROM eheitai_detail_page WHERE eheitai_id = ?", nativeQuery = true)
    List<String> findByGidAndUrl(Integer integer);

    @Query(value = "SELECT img_url FROM eheitai_detail_page WHERE eheitai_id = ?", nativeQuery = true)
    List<String> findByGidAndImgurl(Integer gid);

    ////根据id去查询图片的下载地址和名称
    @Query(value = "SELECT img_url,file_log FROM eheitai_detail_page WHERE eheitai_id = ?",nativeQuery = true)
    List<EheitaiDetailPage> findByGidAndImgurlGetName(Integer gid);
    @Query(value = "SELECT * FROM eheitai_detail_page WHERE eheitai_id = ?",nativeQuery = true)
